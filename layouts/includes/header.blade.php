                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        @if(auth('accounts')->user())
                            ...
                            >
                                @php
                                    $notifications = \TomatoPHP\TomatoNotifications\Models\UserNotification::where('model_id', auth('accounts')->user()->id)->where('model_type', config('tomato-crm.model'))->count();
                                @endphp
                                @if($notifications)
                                    <div class="absolute top-0 font-bold left-6 bg-white border border-gray-500 shadow-sm text-gray-500 rounded-full text-[10px] w-4 h-4 text-center">
                                        {{ $notifications }}
