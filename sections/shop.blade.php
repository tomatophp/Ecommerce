...
<section>
    <x-splade-data default="{showMenu: false}">
        <div class="bg-white">
            <div>
                @include('themes::shop.partials.mobile-filter')

                <main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    @include('themes::shop.partials.header', ['title' => $section['title_'.app()->getLocale()] ?? __('Shop')])
                    ...
                    <section aria-labelledby="products-heading" class="pb-24 pt-6">
                        ...
                        <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                            <x-splade-form method="GET" action="{{url('shop')}}" submit-on-change>
                                @include('themes::shop.partials.web-filter', ['options' => $options])
                            </x-splade-form>
                                    ...
                                    <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
                                        ...
                                        <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
                                            @foreach($products as $product)
                                                @include('themes::shop.partials.product-card')
                                            @endforeach
                                        </div>

                                        <div class="my-4">
                                            {!! $products->links('themes::sections.pagination') !!}
                                        </div>
                                    </div>
                                </div>
                            </div>
